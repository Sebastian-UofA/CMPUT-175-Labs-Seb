...
def main():
    ...
    for t in range(time_steps):
        ...
        if job and job.high_priority():
            high_priority_queue.enqueue(job)  #If the job has high priority (job.high_priority() returns True), it is enqueued into the high_priority_queue.
        elif job:
            low_priority_queue.enqueue(job) #If the job has low priority, it is enqueued into the low_priority_queue.
        ...
        ########################
        if not process_running:   #If the processor is idle (process_running is False), a new job is dequeued from the appropriate queue:
            ########################
            ### ENTER YOUR CODE ####
            ########################
            ## Remove the pass
            ...
            if not high_priority_queue.isEmpty():            #If the high_priority_queue is not empty, a job is dequeued from it and set as the current job. The process_running flag is set to True.
                current_job = high_priority_queue.dequeue()
                process_running = True
            elif not low_priority_queue.isEmpty():
                current_job = low_priority_queue.dequeue()  #If the high_priority_queue is empty and the low_priority_queue is not empty, a job is dequeued from the low_priority_queue and set as the current job. The process_running flag is set to True.
                process_running = True
